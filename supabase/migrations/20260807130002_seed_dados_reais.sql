-- ============================================================================
-- Substitui o seed inicial (migration 20260807120003) pelos valores REAIS:
-- a tabela oficial de pontuação do Edital 3.244/2025 (Diário Oficial da
-- União, Seção 3, 27/11/2025, pág. 64-65) e as experiências reais do
-- currículo do candidato, já classificadas por quesito/item.
--
-- Itens de docência (carga horária) e de vínculo profissional/pesquisa
-- (período) usam o cálculo automático definido na migration
-- 20260807130001 — as fontes abaixo informam horas ou datas, não valor.
--
-- Idempotência: mesma lógica do seed anterior (não roda de novo se já
-- existir um edital com esse nome para o usuário). Como o seed antigo já
-- pode ter rodado com valores estimados incorretos, é preciso apagar o
-- edital antigo antes de rodar este (delete cascade em editais remove
-- quesitos/itens/fontes/áreas/ações vinculados) e então chamar
-- seed_dados_iniciais() de novo pela UI.
-- ============================================================================
...
begin
  ...
  insert into public.fontes_pontuacao (item_id, descricao, carga_horaria_horas, confirmado, data_inicio, data_fim) values
    (v_item_docencia_grad_sc, 'Professor voluntário — EMI025 Saúde Coletiva — Escola de Enfermagem/UFMG (2021/02 e 2022/01)', 120, true, '2021-08-01', '2022-06-30'),
    (v_item_docencia_grad_sc, 'Professor voluntário — EMI041 Epidemiologia — Escola de Enfermagem/UFMG (2022/02)', 60, true, '2022-08-01', '2022-12-20'),
    (v_item_docencia_grad_sc, 'Professor convidado — MED065 Iniciação à Atenção Primária à Saúde I — Faculdade de Medicina/UFMG (2024/01)', 75, true, '2024-02-01', '2024-06-30'),
    (v_item_docencia_grad_sc, 'Professor assistente substituto — ENA019 TA Gestão do Sistema de Saúde — Escola de Enfermagem/UFMG (2023/02)', 75, true, '2023-08-01', '2023-12-20'),
    (v_item_docencia_grad_sc, 'Estágio docente — MED065 Iniciação à Atenção Primária à Saúde I — Faculdade de Medicina/UFMG (2023/02)', 75, true, '2023-08-01', '2023-12-20');
  ...
  insert into public.fontes_pontuacao (item_id, descricao, carga_horaria_horas, confirmado, data_inicio, data_fim) values
    (v_item_docencia_grad_outras, 'Professor titular — ENF20252N Primeiros Socorros — Faculdade CTA (2025/02)', 80, true, '2025-08-01', '2025-12-20'),
    (v_item_docencia_grad_outras, 'Professor assistente substituto — ENA035 Capacitação Pedagógica Aplicada à Enfermagem — Escola de Enfermagem/UFMG (2023/02)', 60, true, '2023-08-01', '2023-12-20'),
    (v_item_docencia_grad_outras, 'Professor assistente substituto — ENA035 Seminário de Integração — Escola de Enfermagem/UFMG (2023/02)', 15, true, '2023-08-01', '2023-12-20');
  ...
  -- é o mesmo. Ajustar manualmente as datas se algum dia a soma sem
  -- deduplicação ficar abaixo do teto.
  insert into public.fontes_pontuacao (item_id, descricao, confirmado, data_inicio, data_fim) values
    (v_item_ativ_prof_saude, 'Referência Técnica — Coordenação do Serviço de Atenção Domiciliar, Gerência de Urgência e Emergência — Prefeitura de Belo Horizonte. 18/08/2025–atual.', true, '2025-08-18', null),
    (v_item_ativ_prof_saude, 'Enfermeiro Especialista em Estética Avançada — Instituto Antony Barbosa. 20h/semana. 01/01/2024–15/08/2025.', true, '2024-01-01', '2025-08-15'),
    (v_item_ativ_prof_saude, 'Enfermeiro Especialista em Políticas e Gestão em Saúde — Secretaria Estadual de Saúde de Minas Gerais. 40h/semana. 20/06/2022–23/03/2023.', true, '2022-06-20', '2023-03-23'),
    (v_item_ativ_prof_saude, 'Enfermeiro CTI Covid — Fundação Hospitalar São Francisco de Assis. 06/04/2021–17/05/2021.', true, '2021-04-06', '2021-05-17'),
    (v_item_ativ_prof_saude, 'Enfermeiro — EPC Comunicação e Educação em Saúde S/A. 15/03/2021–14/05/2021.', true, '2021-03-15', '2021-05-14');
  ...
  insert into public.fontes_pontuacao (item_id, descricao, confirmado, data_inicio, data_fim)
  values (
...
-- ============================================================================
-- Atualiza a duplicação de edital para também copiar a configuração de
-- cálculo automático dos itens (senão um novo edital perderia o modo
-- carga_horária/período dos itens copiados).
-- ============================================================================
...
begin
  ...
  loop
    ...
    loop
      insert into public.itens_barema (
        quesito_id, nome, unidade, pontuacao_maxima,
        modo_calculo, horas_por_unidade, pontos_por_unidade, unidade_periodo
      )
      values (
        v_novo_quesito_id, v_item.nome, v_item.unidade, v_item.pontuacao_maxima,
        v_item.modo_calculo, v_item.horas_por_unidade, v_item.pontos_por_unidade, v_item.unidade_periodo
      );
    end loop;
  end loop;
